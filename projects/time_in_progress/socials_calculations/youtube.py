...
data_dir = os.path.join(data_path, "youtube")
...
TODO
TODO
TODO: Rebuild and clean this, its wild 
"""
...
def get_graph_data(account, set_range, set_length):
    ...
    for data_date in chosen_data:
        ...
        name = value["name"]
        video_count = int(value["stats"]["video_count"])
        subscriber_count = int(value["stats"]["subscriber_count"])
    ...
    for data_date in chosen_data:
        ...
        for prev, prev_value in data.items():
            ...
            if start_time <= time:
                ...
                date = prev
                past_video_count = int(prev_value["stats"]["video_count"])
                past_subscriber_count = int(
                    prev_value["stats"]["subscriber_count"])

                # Append when posted a new post.
                if past_video_count != prev_post and prev_post != None:
                    posted_at_subscriber_count = past_subscriber_count
                else:
                    posted_at_subscriber_count = 0
                prev_post = past_video_count

                collection.append({prev: prev_value})
                follower_count.append(past_subscriber_count)

                value_data = maybe_append(date, set_range,  set_length)

                percent = (int(prev_data) / int(past_subscriber_count)) * 100
                growth = 100 - percent
                prev_data = past_subscriber_count
                ...
                if growth < 80:
                    if value_data not in no_duplicates_list:
                        ...
                        clean.append({
                            'name': past_name,
                            'post': past_video_count,
                            'subscriber': past_subscriber_count,
                            'date': date,
                            'postedAt': posted_at_subscriber_count
                        })

                        # print(value_data, past_subscriber_count)
                        # print(time.strftime("%Y-%m-%d %H:%M"), "| diff:", time - start_time, " | ",
                        # int(past_subscriber_count),'-', int(subscriber_count), "=",
                        # int(subscriber_count) - int(past_subscriber_count))
...
def get_difference(account, set_range, set_length):
    ...
    for data_date in chosen_data:
        ...
        video_count = int(value["stats"]["video_count"])
        subscriber_count = int(value["stats"]["subscriber_count"])
        total_views = int(value["stats"]["view_count"])
    ...
    for data_date in chosen_data:
        ...
        for prev, prev_value in data.items():
            ...
            if start_time <= time:
                ...
                past_subscriber_count = int(
                    prev_value["stats"]["subscriber_count"])
                past_total_views = int(prev_value["stats"]["view_count"])

                print(int(prev_value["stats"]["view_count"]))
                ...
                if growth < 80:
                    ...
                    if value_data not in no_duplicates_list:
                        ...
                        # Append when posted a new post.
                        if past_video_count != prev_post and prev_post != None:
                            print('yeaaah', past_subscriber_count)
                            posted_at_subscriber_count = past_subscriber_count
                        else:
                            posted_at_subscriber_count = None
                        prev_post = past_video_count
    ...
    for t in prev_time:
        ...
        past_video_count = int(prev_time[t]["stats"]["video_count"])
        past_subscriber_count = int(prev_time[t]["stats"]["subscriber_count"])
        past_total_views = int(prev_time[t]["stats"]["view_count"])
        ...
        post_difference = int(video_count) - int(past_video_count)
        subscriber_difference = int(
            subscriber_count) - int(past_subscriber_count)
        print('\nSummary:')
        ...
        print(
            ...
            time_difference, "|",
            int(past_subscriber_count), '-',
            int(subscriber_count), '=',
            subscriber_difference,
        ...
        data = {
            ...
            "to_date": time,
            "latest_video_count": int(video_count),
            "latest_subscriber": int(subscriber_count),

            "from_date": t_date,
            "past_video_count": int(past_subscriber_count),
            "past_subscriber_count": past_subscriber_count,
            "past_total_views": past_total_views,

            "total_views": total_views,
            "post_difference": post_difference,
            "subscriber_difference": subscriber_difference,
