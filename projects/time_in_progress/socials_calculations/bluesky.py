...
data_dir = os.path.join(data_path, "bluesky")
...
def get_graph_data(account, set_range, set_length):
    ...
    for data_date in chosen_data:
        ...
        name = value["name"]
        posts_count = int(value["stats"]["posts_count"])
        followers_count = int(value["stats"]["followers_count"])
    ...
    for data_date in chosen_data:
        ...
        for prev, prev_value in data.items():
            ...
            if start_time <= time:
                ...
                date = prev
                past_posts_count = int(prev_value["stats"]["posts_count"])
                past_followers_count = int(
                    prev_value["stats"]["followers_count"])

                # Append when posted a new post.
                if past_posts_count != prev_post and prev_post != None:
                    posted_at_followers_count = past_followers_count
                else:
                    posted_at_followers_count = 0
                prev_post = past_posts_count

                collection.append({prev: prev_value})
                follower_count.append(past_followers_count)

                value_data = maybe_append(date, set_range,  set_length)

                percent = (int(prev_data) / int(past_followers_count)) * 100
                growth = 100 - percent
                prev_data = past_followers_count
                ...
                if growth < 80:
                    if value_data not in no_duplicates_list:
                        ...
                        clean.append({
                            'name': past_name,
                            'post': past_posts_count,
                            'followers': past_followers_count,
                            'date': date,
                            'postedAt': posted_at_followers_count
                        })

                        # print(value_data, past_followers_count)
                        # print(time.strftime("%Y-%m-%d %H:%M"), "| diff:", time - start_time, " | ",
                        # int(past_followers_count),'-', int(followers_count), "=",
                        # int(followers_count) - int(past_followers_count))
...
def get_difference(account, set_range, set_length):
            ...
            if start_time <= time:
                ...
                if growth < 80:
                    ...
                    if value_data not in no_duplicates_list:
                        ...
                        # Append when posted a new post.
                        if past_posts_count != prev_post and prev_post != None:
                            print('yeaaah', past_followers_count)
                            posted_at_followers_count = past_followers_count
                        else:
    ...
    for t in prev_time:
        ...
        past_posts_count = int(prev_time[t]["stats"]["posts_count"])
        past_followers_count = int(prev_time[t]["stats"]["followers_count"])
        ...
        post_difference = int(posts_count) - int(past_posts_count)
        followers_difference = int(followers_count) - int(past_followers_count)
        print('\nSummary:')
        ...
        data = {
            ...
            "to_date": time,
            "latest_posts_count": int(posts_count),
            "latest_followers": int(followers_count),

            "from_date": t_date,
            "past_post_value": int(past_followers_count),
            "past_followers_value": past_followers_count,
