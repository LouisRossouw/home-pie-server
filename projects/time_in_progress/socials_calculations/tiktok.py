...
data_dir = os.path.join(data_path, "tiktok")
...
def set_stats_manual(likes, followers_value, following_value):
    """ Manualy set stats """
    ...
    data = {
        'likes': int(likes),
        'followers_value': int(followers_value),
        'following_value': int(following_value)
    }

    save_data('time.in.progress', data)


def save_data(account_name, stats):
    """ Records data for YouTube channel, to be used later
        for matplot lib etc. """
    ...
    print(str(date_now_full), "Formatting and saving data")

    data_log_dir = os.path.join(
        f"{data_dir}", str(account_name))
...
def get_graph_data(account, set_range, set_length):
    ...
    for data_date in chosen_data:
        ...
        name = value["name"]
        post_value = int(value["stats"]["likes"])
    ...
    for data_date in chosen_data:
        ...
        for prev, prev_value in data.items():
            ...
            if start_time <= time:
                ...
                date = prev
                past_post_value = int(prev_value["stats"]["likes"])
...
def get_difference(account, set_range, set_length):
    ...
    for t in prev_time:
        ...
        past_post_value = int(prev_time[t]["stats"]["likes"])
